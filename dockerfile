...
&& yum install -y readline-devel && yum install -y tk-devel && yum install -y gcc && yum install -y make && yum install -y libffi-devel && yum install -y wget \
...
&& wget https://www.python.org/ftp/python/3.8.1/Python-3.8.1.tgz && tar -zxvf Python-3.8.1.tgz && cd Python-3.8.1 && ./configure prefix=/usr/local/python3.8 --enable-shared --enable-optimizations LDFLAGS="-Wl,--rpath=/usr/local/python3.8/lib" \
