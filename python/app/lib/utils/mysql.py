...
class Mysql_db:
    ...
    def target_scan_list(self, username, pagenum, pagesize):
        ...
        start = (int(pagenum)-1) * int(pagesize)
        pagesize = int (pagesize)
        sql = "select scan_id, target, description, scan_time, scan_status, scan_schedule, vulner_number from target_scan where username = %s and trash_flag = '0' order by scan_id + 0 desc limit %s, %s"
