...
from app.thirdparty.dirsearch.lib.connection import Requester, RequestException
from app.thirdparty.dirsearch.lib.core import Dictionary, Fuzzer, ReportManager
from app.thirdparty.dirsearch.lib.reports import JSONReport, XMLReport, PlainTextReport, SimpleReport, MarkdownReport, CSVReport
from app.thirdparty.dirsearch.lib.utils import FileUtils
...
class Controller(object):
    ...
    def matchCallback(self, path):
        ...
        if (
            ...
            if path.response.status in [301, 302, 303, 307, 308]:
                temp = (path.response.headers["location"], 200)
            else:
                temp = (path.path, path.response.status)
