        ...
        if kwargs['scan_ip'] and '3' in scan_option:
            ...
            for ip in ip_list:
                if scan_set['scanner'] == 'nmap':
                    scan_list = scan_list + self.port_scan.nmap_scan(kwargs['username'], kwargs['target'], kwargs['domain'], ip, kwargs['scan_id'], scan_set['nmap_cmd'], scan_set['port'])
                else:
                    scan_list = scan_list + self.port_scan.masscan_scan(kwargs['username'], kwargs['target'], kwargs['domain'], ip, kwargs['scan_id'], scan_set['masscan_cmd'], scan_set['port'], scan_set['rate'])
            if '3' in scan_option:
